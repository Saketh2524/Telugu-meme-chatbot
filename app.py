...
def generate_main_response(user_query, history_str, main_meme_context):
    ...
    Your task is to create a concise "Tanglish" response that integrates the provided Telugu meme.
    ...
    *** YOUR TASK ***
    ...
    Your response must be a single, punchy sentence. DO NOT ask a follow-up question. DO NOT repeat the user's query.
    """
    response = model.generate_content(prompt)
...
def generate_probing_question(user_query, main_response, other_meme_contexts):
    ...
    *** YOUR TASK ***
    Your task is to now write a short, witty, probing follow-up question in simple ENGLISH that is inspired by the ideas in the "OTHER related memes". The question should feel like a natural continuation.
    """
    response = model.generate_content(prompt)
    return response.text
